...
func main() {
	...
	router.GET("/stream", HeadersMiddleware(), stream.SSEConnMiddleware(), func(gctx *gin.Context) {
		...
		gctx.Stream(func(w io.Writer) bool { //streaming system
			// Stream data to client
			for { //loop infinitely
				select {
				// Send msg to the client
				case msg, ok := <-client.Channel: //blocking stream read??? Wait until there is something in the channel to send
					if !ok {
						return false
					}
					gctx.SSEvent("message", msg) //actually send the event!
					return true
				// Client exit
				case <-gctx.Request.Context().Done(): //closes channel
					return false
				}
			}
		})
